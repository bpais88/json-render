...
export function Widget({
  ...
  const { spec, isStreaming, error, send, clear } = useUIStream({
    ...
    onComplete: async (completedSpec) => {
      // Save to database when generation completes (skip for example pages)
      if (skipSave) return;
  ...
  // Keep a ref to the latest send so the auto-generate timer always uses it
  const sendRef = useRef(send);
  useEffect(() => {
    sendRef.current = send;
  }, [send]);

  // Auto-generate on mount when autoGenerate is true
  useEffect(() => {
    if (!autoGenerate || !initialPrompt) return;
    const timer = setTimeout(() => {
      sendRef.current(initialPrompt, { data: {} });
    }, 150);
